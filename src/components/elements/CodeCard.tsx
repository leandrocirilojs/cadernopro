...
export const CodeCard: React.FC<CodeCardProps> = ({
  ...
  return (
    <div 
      ...
      className="bg-slate-900 border border-slate-700 rounded-2xl p-3.5 shadow-lg flex flex-col relative text-slate-100 transition-all"
      style={{ 
        width: isDraggable ? `${element.width || 440}px` : '100%',
    ...
    >
      ...
      {/* Code Textarea */}
      <div className="relative rounded-lg overflow-hidden border border-slate-800 bg-slate-950 flex-1 flex flex-col min-h-[120px]">
        <textarea
          value={code}
          onChange={(e) => setCode(e.target.value)}
          onBlur={handleBlur}
          placeholder="// Cole ou escreva seu código aqui..."
          className="w-full flex-1 p-2.5 font-mono text-xs text-purple-200 bg-transparent resize-none outline-none leading-relaxed selection:bg-purple-900 selection:text-white"
          spellCheck={false}
        />
      </div>
      ...
      {isDraggable && (
        <ResizeHandle
          ...
          initialWidth={element.width || 440}
